...
CUDPP_DLL
CUDPPResult
cudppCompactingHashGetSize(CUDPPHandle plan, unsigned int * size)
{
    hti_void * hti_init = (hti_void *) getPlanPtrFromHandle<hti_void>(plan);
    if (hti_init->config.type != CUDPP_COMPACTING_HASH_TABLE)
    {
        // better be a MULTIVALUE
        return CUDPP_ERROR_ILLEGAL_CONFIGURATION;
    }
    hti_compacting * hti = 
        (hti_compacting *) getPlanPtrFromHandle<hti_compacting>(plan);
    *size = hti->hash_table->get_unique_keys_size();
    return CUDPP_SUCCESS;
}
...
CUDPP_DLL
CUDPPResult
cudppMultivalueHashGetIndexCounts(CUDPPHandle plan, uint2 ** d_index_counts, unsigned *num_unique_keys_build)
{
    hti_void * hti_init = (hti_void *) getPlanPtrFromHandle<hti_void>(plan);
    if (hti_init->config.type != CUDPP_MULTIVALUE_HASH_TABLE)
    {
    ...
    hti_multivalue * hti = 
        (hti_multivalue *) getPlanPtrFromHandle<hti_multivalue>(plan);
    // @TODO fix up constness
    *d_index_counts = (uint2*) (hti->hash_table->get_index_counts());
    *num_unique_keys_build = (hti->hash_table->get_num_unique_keys_build());
    return CUDPP_SUCCESS;
}
/** @} */ // end Plan Interface
/** @} */ // end publicInterface
